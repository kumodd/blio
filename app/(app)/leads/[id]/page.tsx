...
export default async function LeadDetailPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  const leadPhone = lead.business.phone ?? contacts.find((contact) => contact.type === "phone" || contact.type === "whatsapp")?.value;
  ...
  return <>
    ...
    <div className="topbar"><div><p className="eyebrow">Prospect intelligence</p><h1>{lead.business.name}</h1><p className="subtitle">{lead.business.category} · {lead.business.city ?? lead.business.address}</p></div><div className="actions"><StatusBadge status={lead.status} /><a className="button button-primary" href="#outreach"><MessageCircle /> Create outreach</a></div></div>
    <div className="detail-layout">
      <div className="card detail-card">
        ...
        <div className="detail-section"><h3><MapPin /> Business facts</h3><div className="fact-grid"><div className="fact"><div className="fact-label">Business type</div><div className="fact-value">{lead.business.category}</div></div><div className="fact"><div className="fact-label">Rating</div><div className="fact-value">{lead.business.rating ? `${lead.business.rating} / 5 · ${lead.business.reviewCount ?? 0} reviews` : "Not available"}</div></div><div className="fact"><div className="fact-label">Website</div><div className="fact-value">{lead.business.website ? <a className="text-link" href={safeUrl(lead.business.website)} target="_blank" rel="noreferrer">Open website <ExternalLink size={12} style={{ verticalAlign: "-2px" }} /></a> : "Not found"}</div></div><div className="fact"><div className="fact-label">Contact paths</div><div className="fact-value">{contacts.length || leadPhone ? `${contacts.length + (leadPhone && !contacts.some((contact) => contact.value === leadPhone) ? 1 : 0)} found` : "None found"}</div></div><div className="fact" style={{ gridColumn: "1 / -1" }}><div className="fact-label">Description</div><div className="fact-value">{lead.business.description ?? "No description collected."}</div></div></div></div>
        <div className="detail-section"><h3><MessageCircle /> Contact paths</h3><div className="contact-list">{contacts.length || lead.business.website ? <>{contacts.map((contact) => { const href = contact.type === "phone" ? phoneUrl(contact.value) : contact.type === "whatsapp" ? whatsappUrl(contact.value) : contact.type === "sms" ? smsUrl(contact.value) : contact.type === "email" ? `mailto:${contact.value}` : safeUrl(contact.value); const Icon = contact.type === "phone" ? Phone : contact.type === "email" ? Mail : contact.type === "instagram" ? Instagram : contact.type === "website" ? Globe : MessageCircle; return <div className="contact-row" key={`${contact.type}-${contact.value}`}><Icon /><a href={href} target={contact.type === "phone" || contact.type === "sms" || contact.type === "email" ? undefined : "_blank"} rel="noreferrer">{contact.value}</a>{contact.verified ? <span className="help" style={{ marginLeft: "auto" }}>Verified</span> : null}</div>; })}{lead.business.website && !contacts.some((contact) => contact.type === "website" || contact.value === lead.business.website) ? <div className="contact-row"><Globe /><a href={safeUrl(lead.business.website)} target="_blank" rel="noreferrer">{lead.business.website}</a></div> : null}</> : <p className="help">No public contact path was found.</p>}</div></div>
      ...
      <div id="outreach" style={{ display: "grid", gap: 18, alignContent: "start" }}><OutreachComposer leadId={lead.id} phone={leadPhone} email={contactEmail} instagram={instagram} sellerProfile={campaign?.context.sellerProfile} initialDrafts={drafts} /><LeadActions lead={lead} /></div>
    </div>
  </>;
}
