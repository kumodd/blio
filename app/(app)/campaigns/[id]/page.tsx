...
export default async function CampaignPage({ params, searchParams }: { params: Promise<{ id: string }>; searchParams: Promise<{ autostart?: string }> }) {
  ...
  return (
    <>
      ...
      <section className="card section-card" style={{ marginTop: 18 }}><div className="section-heading"><div><h2>Target and opportunity brief</h2><p>This context grounds scoring and personalized outreach.</p></div></div><div className="fact-grid"><div className="fact"><div className="fact-label">What you sell</div><div className="fact-value">{campaign.context.offer}</div></div><div className="fact"><div className="fact-label">Ideal customer</div><div className="fact-value">{campaign.context.targetCustomer}</div></div><div className="fact"><div className="fact-label">Likely problem</div><div className="fact-value">{campaign.context.painPoint}</div></div><div className="fact"><div className="fact-label">Service opportunity</div><div className="fact-value">{campaign.context.valueProposition}</div></div></div>{campaign.context.sellerProfile ? <div className="seller-profile-summary"><div className="seller-profile-icon"><Globe size={17} /></div><div><div className="fact-label">Sender profile used in outreach</div><strong>{campaign.context.sellerProfile.name ?? campaign.context.sellerProfile.websiteUrl}</strong><p>{campaign.context.sellerProfile.services?.slice(0, 4).join(" · ") ?? campaign.context.sellerProfile.description ?? "Website context captured for personalized outreach."}</p><a className="text-link" href={campaign.context.sellerProfile.websiteUrl} target="_blank" rel="noreferrer">Open your website <ArrowRight size={13} style={{ verticalAlign: "-2px" }} /></a></div></div> : <div className="seller-profile-summary seller-profile-missing"><div className="seller-profile-icon"><Globe size={17} /></div><div><strong>Add your website to improve outreach relevance.</strong><p>blio will capture your business identity, services, and available contact details for the sender context.</p><Link className="text-link" href={`/campaigns/${campaign.id}/edit`}>Add website <ArrowRight size={13} style={{ verticalAlign: "-2px" }} /></Link></div></div>}</section>
    </>
  );
}
