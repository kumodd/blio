...
export const metadata: Metadata = {
  title: "Pricing",
  description: "Simple early-access pricing for blio AI prospecting and outreach.",
};
...
export default function PricingPage() {
  return <div className="pricing-page"><section className="public-hero pricing-hero"><p className="eyebrow">Pricing</p><h1>Start focused. Scale when your pipeline earns it.</h1><p className="public-lead">Use blio to find, understand, and prioritize target clients before you commit to a larger sales workflow.</p><div className="pricing-note"><Sparkles size={16} /><span>Early-access note: paid checkout is not active yet. Current users can start with the free workspace while plans are being finalized.</span></div></section><section className="pricing-grid" aria-label="Pricing plans">{plans.map((plan) => <article className={`pricing-card ${plan.featured ? "pricing-card-featured" : ""}`} key={plan.name}>{plan.featured ? <span className="pricing-badge">Most useful for growth</span> : null}<div className="pricing-card-top"><div><h2>{plan.name}</h2><p>{plan.description}</p></div></div><div className="pricing-price"><strong>{plan.price}</strong><span>{plan.detail}</span></div><ul>{plan.features.map((feature) => <li key={feature}><Check size={15} />{feature}</li>)}</ul><Link className={`button ${plan.featured ? "button-primary" : "button-secondary"}`} href={plan.name === "Scale" ? "/contact" : "/login"}>{plan.name === "Scale" ? "Talk to us" : "Start with blio"} <ArrowRight /></Link></article>)}</section><section className="public-callout pricing-callout"><div><p className="eyebrow">No lock-in during early access</p><h2>Build a useful prospecting habit first.</h2><p>Plans and limits may change before paid billing launches. We’ll make pricing and cancellation terms clear before charging for a subscription.</p></div><Link className="button button-secondary" href="/refund-cancellation">Read billing policy <ArrowRight /></Link></section></div>;
}
