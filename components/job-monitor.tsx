...
export function JobMonitor({ initialJob }: { initialJob: ResearchJob }) {
  ...
  return <div className="card job-card">
    ...
    {job.diagnostics?.length ? <div className="job-diagnostics"><strong>Research notes</strong>{job.diagnostics.map((diagnostic, index) => <div className={`job-diagnostic job-diagnostic-${diagnostic.severity}`} key={`${diagnostic.code}-${index}`}><span>{diagnostic.message}</span></div>)}</div> : null}
